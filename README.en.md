...
Pi sessions form a tree of records: retried prompts or `/tree` navigation leave abandoned branches behind. Valid sessions follow Pi's resume rule and migrate only the active branch from the last record back to the root via `parentId`; cycles or broken links stop safely at the confirmed path so a damaged file cannot block migration. Abandoned branches are counted as skipped records. Active-branch details from before context compaction remain available, with the summary migrated as a marker.

Claude Code sessions are likewise append-only JSONL message trees: rewinds and retries leave abandoned `parentUuid` forks. Valid transcripts follow Claude's `--resume` semantics by walking the active chain from the latest non-sidechain message and applying compact before snip; incomplete compact metadata is handled conservatively by retaining history. Snipped ranges and abandoned forks are counted as skipped records, and `compact_boundary` is kept as a marker.

Codex rollouts are append-only JSONL: a user rollback appends `thread_rolled_back`. Agent Connect applies rollback to confirmed real user turns: legacy `user_message` and paginated `item_completed(UserMessage)` events are preferred, with non-contextual ResponseItem or inter-agent boundaries used as fallbacks and dual-track records deduplicated. Dropped turns are counted in the migration report. Unlike Codex resume's model-context replacement, migration keeps the full pre-compaction history and stores the compaction summary as a marker.
