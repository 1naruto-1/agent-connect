...
`listSessions()` and `readSession()` must resolve a title through the shared helpers in `src/title.ts`, because `readSession().title` becomes the session name in the target Harness. The order is: the explicit title the Harness stored (`custom-title` / `ai-title`, `session_info.name`, `composer.name`) → the first user message that is neither an `[agent-connect]` provenance marker nor a Harness-injected wrapper such as `<command-name>` → `untitledSession()`, which names the source Harness instead of a bare session id. For Claude Code, `custom-title` wins over `ai-title`. `listSessions()` skips the last step, since the id is already in the row.

## Native storage integrations

| Tool | Session store | Implementation notes |
| --- | --- | --- |
| Claude Code | `~/.claude/projects/*/*.jsonl` | Writes JSONL messages, tool-use/result pairs, and session metadata. `claude-session.ts` ports Claude Code resume semantics from the transcript loader: the reader follows the active `parentUuid` chain from the latest non-sidechain leaf, applies compact relinks before `snipMetadata` removals, and treats abandoned forks as skipped records. Incomplete or disconnected preserved segments conservatively keep history. `compact_boundary` becomes a marker. An explicitly empty `custom-title` shadows older AI titles. On write, tied or out-of-order event timestamps are incremented by milliseconds so Claude resumes from the final record. |
| Codex CLI | `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` | Writes rollout JSONL; reuses configuration details from a recent native Codex session when available. `codex-session.ts` applies `thread_rolled_back` to confirmed user turns: legacy `user_message` and paginated `item_completed(UserMessage)` events are preferred, non-contextual ResponseItem and inter-agent records are fallbacks, and dual-track records are deduplicated. Invalid `num_turns` values do not remove history. Migration intentionally keeps full pre-compaction history and converts the summary to a marker instead of reproducing Codex's replacement model context. |
| Pi | `~/.pi/agent/sessions/--<project>--/*.jsonl` | Sessions are `id`/`parentId` record trees. `pi-session.ts` follows Pi's active-branch rule (last record back to the root), applies v1→v3 format migration in memory only, and treats abandoned branches as skipped records. Cycles and broken parents stop at the confirmed path. Pre-compaction active-branch details remain available and compaction or branch summaries become markers. Writes a session header plus a linked record chain. |
