...
| --- | --- | --- |
| Claude Code | `~/.claude/projects/*/*.jsonl` | Writes JSONL messages, tool-use/result pairs, and session metadata. `claude-session.ts` ports Claude Code resume semantics from the transcript loader: the reader follows the active `parentUuid` chain from the latest non-sidechain leaf, applies `snipMetadata` removals (with parent relinks), and treats abandoned forks as skipped records. `compact_boundary` becomes a marker. Titles prefer `custom-title` over `ai-title`. On write, tied or out-of-order event timestamps are incremented by milliseconds so Claude resumes from the final record. |
| Codex CLI | `~/.codex/sessions/YYYY/MM/DD/rollout-*.jsonl` | Writes rollout JSONL; reuses configuration details from a recent native Codex session when available. `codex-session.ts` aligns with Codex resume's rollback outcome: `thread_rolled_back` drops the newest N user turns from the effective history (counted as skipped). Cut points use adapter-aligned `event_msg` boundaries (`user_message` / paginated `item_completed`+`UserMessage`), matching how this reader surfaces user text (not a line-for-line port of ResponseItem turn boundaries in `thread_rollout_truncation.rs`). Compaction becomes a marker (summary text preserved when present). |
